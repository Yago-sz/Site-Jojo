...
let timeToWait = 30000; // Tempo de espera entre sorteios em milissegundos (30 segundos)
let isCooldown = false; // Flag para verificar se o tempo de espera está ativo
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    ...
    firebase.database().ref('users/' + userId).once('value').then(snapshot => {
      if (snapshot.exists() && snapshot.val().stand) {
        ...
        // Se o tempo de espera já estiver ativo, alerta o usuário e mostra o tempo restante
        if (isCooldown) {
          const remainingTime = Math.max(0, timeToWait - (Date.now() - snapshot.val().lastSorteio));
          alert(`Aguarde ${Math.ceil(remainingTime / 1000)} segundos antes de tentar novamente.`);
          return;
        }
      ...
      // Se não há stand sorteado ou está no cooldown, continua com a escolha
      if (videoInProgress) {
      ...
      if (isCooldown) {
        alert("Aguarde o tempo de cooldown antes de sortear outro Stand.");
        return; // Impede o sorteio se estiver no cooldown
      }
        ...
        animacaoStand.onloadedmetadata = () => {
          ...
          setTimeout(() => {
            ...
            firebase.database().ref('users/' + userId).set({
              ...
              video: videoPath,
              lastSorteio: Date.now() // Salva o timestamp do último sorteio
            }).then(() => {
